...
export type ADropdownItem = { id: string; name: string; [prop: string]: any };
    ...
    providers: [
        {
            ...
            useExisting: forwardRef(() => ADropdownComponent),
            multi: true
        }
    ]
})
export class ADropdownComponent implements OnChanges, AfterViewInit, ControlValueAccessor {
    ...
    @ViewChild('ref', { static: true }) private reference: ElementRef<HTMLDivElement>;
    /** Template Reference for the dropdown tooltip contents */
    @ViewChild(TemplateRef, { static: true }) private dropdown_tooltip: TemplateRef<any>;
    ...
    @ViewChild('viewport', { static: true }) private scroll_el: ElementRef;

    public ngOnChanges(changes: SimpleChanges): void {
        if (changes.items) {
            this.list = this.items.map((v) => (v instanceof Object ? (v as ADropdownItem) : { id: v, name: v }));
